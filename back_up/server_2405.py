...
async def deploy_bot(request: Request):
    """
    Принимает JSON-граф от конструктора и загружает его в Оркестратор.
    """
    try:
        ...
        # Загружаем настройки (здесь можно добавить чтение токена из БД)
        # Пока используем заглушку для примера
        config = {"messenger_type": "telegram", "token": "PLACEHOLDER_TOKEN"}

        # Загружаем конфигурацию
        bot_orchestrator.load_bot_config(bot_id, config)

        print(f"[SYSTEM] Bot '{bot_id}' развернут. Узлов в графе: {len(graph.get('nodes', []))}")
        return {"status": "success", "message": "Бот успешно запущен в Runtime."}
...
@app.post("/api/v1/deploy")
async def deploy_bot(config: BotConfigSchema):
    """
    1. Генерирует код на базе выбранных ID.
    2. Упаковывает в ZIP.
    3. Возвращает путь к пакету.
    """
    try:
        # Генерируем код
        factory = BotFactory(config.selected_module_ids)
        bot_code = factory.generate()

        # Упаковываем
        generator = ProjectGenerator(bot_code, config.selected_module_ids)
        package = generator.create_deployment_package()

        return {
            "status": "success",
            "message": f"Бот '{config.bot_name}' готов к запуску",
            "download_url": "/api/download/bot_package"
        }
    except Exception as e:
